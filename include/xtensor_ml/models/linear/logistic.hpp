...
public:
  explicit LogisticRegression(double gamma, Penalty penalty,
                              bool fit_intercept = true);

  LogisticRegression &fit(const xt::xarray<double> &X,
                          const xt::xarray<double> &y, double lr = 0.01,
                          double tol = 1e-7, size_t max_iter = 1e7);
...
private:
  ...
  double NLL_(const xt::xarray<double> &y,
              const xt::xarray<double> &y_pred) const;
  xt::xarray<double> NLL_grad_(const xt::xarray<double> &X,
                               const xt::xarray<double> &y,
                               const xt::xarray<double> &Y_pred) const;
  static xt::xarray<double> sigmoid_(const xt::xarray<double> &z);
