...
TEST(LogisticRegressionTest, SigmoidTest) {
  ...
  xt::xarray<double> expected = {
      0.0, 0.5,
      1.0}; // Sigmoid of large negative -> 0, 0 -> 0.5, large positive -> 1
...
TEST(LogisticRegressionTest, NLLTest) {
  ...
  double expected_nll =
      -(xt::sum(y * xt::log(y_pred) + (1.0 - y) * xt::log(1.0 - y_pred)))();
...
protected:
  void SetUp() override {
    X_train =
        xt::xarray<double>{{1.0, 2.0}, {2.0, 3.0}, {3.0, 4.0}, {4.0, 5.0}};
...
TEST_F(LogisticRegressionFTest, PredictTest) {
  ...
  for (size_t i = 0; i < predictions.size(); ++i) {
    EXPECT_TRUE(predictions(i) >= 0.0 &&
                predictions(i) <=
                    1.0); // Logistic regression predicts probabilities
  }
}
} // namespace linear_models
} // namespace xtensor_ml
